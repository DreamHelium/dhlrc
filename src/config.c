...
file_changed_cb ()
{
    ...
    gboolean get_file
        = g_file_load_contents (file, NULL, &content_val, NULL, NULL, NULL);
    if (!get_file)
        g_error ("Get file failed!");
    content = cJSON_Parse (content_val);
    g_free (content_val);
}

static void
monitor_task (GTask *task, gpointer source_object, gpointer task_data,
    ...
    if (monitor)
        {
            ...
            g_signal_connect (monitor, "changed", G_CALLBACK (file_changed_cb),
                              NULL);
            /* Make the "content" */
            gchar *content_val = NULL;
            gboolean get_file = g_file_load_contents (file, NULL, &content_val,
                                                      NULL, NULL, NULL);
            if (!get_file)
                g_error ("Get file failed!");
            content = cJSON_Parse (content_val);
...
dhlrc_make_config ()
{
    if (!config_file)
        config_file = g_strconcat (g_get_user_config_dir (),
                                   "/dhlrc/config.json", NULL);
    if (!dh_file_exist (config_file))
        {
#ifdef __APPLE__
            gchar *minecraft_dir
                = g_strconcat (g_get_home_dir (),
                               "/Library/Application Support/minecraft", NULL);
            ...
            gchar *cache_dir = g_strconcat (g_get_user_cache_dir (),
                                            G_DIR_SEPARATOR_S, "dhlrc", NULL);
            ...
            gchar *recipe_dir
                = g_strconcat ("recipes", G_DIR_SEPARATOR_S, NULL);
            cJSON_AddStringToObject (config, "recipeConfig", recipe_dir);
            cJSON_AddStringToObject (config, "itemTranslate",
                                     "translation.json");
...
char *
dh_get_config_item (const char *item)
{
...
dh_get_cache_dir ()
{
    ...
    if (ret)
        ...
        return g_strconcat (g_get_user_cache_dir (), "/dhlrc", NULL);
}
...
void
dh_set_or_create_item (const char *item, const char *val, gboolean save)
{
