    ...
    : QWidget (parent)
{
    ...
    if (r)
        ...
        QMessageBox::critical (parent, _ ("Not supported recipe!"),
                               _ ("The recipe is not supported!"));
}
                    ...
                    if (pos != -1)
                        {
                            ...
                            for (int i = 0; i < r->pt[pos].item_string->num;
                                 i++)
                                labelStr
                                    += "  "
                                       + QString (trm (
                                           r->pt[pos].item_string->val[i]))
                                       + "\n";
...
QString
RecipesShowUI::getPicFilename (const char *item)
{
    return dh::findIcon (item);
}

QVBoxLayout *
RecipesShowUI::getRecipePic (DhRecipes *r, QStringList &p)
{
                    ...
                    if (!dir.isEmpty ())
                        {
                            ...
                            label->setToolTip (
                                trm (r->pt[pos].item_string->val[0]));
                        }
                    else
                        {
