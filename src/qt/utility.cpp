...
dh::loadRegion (QWidget *parent)
{
  ...
  if (ret == QDialog::Accepted)
    {
      dh::loadRegion (parent,
                      dh_info_get_uuid (DH_TYPE_NBT_INTERFACE_CPP)->val[0]);
    }
...
dh::loadRegion (QWidget *parent, const char *uuid)
{
          ...
          if (region)
            {
              auto str = QInputDialog::getText (
                  ...
                  _ ("Enter name for the new Region."), QLineEdit::Normal,
              ...
              if (str.isEmpty ())
                ...
                dh_info_new (DH_TYPE_REGION, region,
                             g_date_time_new_now_local (), str.toLocal8Bit (),
                             nullptr, nullptr);
            }
              ...
              if (region)
                {
                  auto str = QInputDialog::getText (
                      parent, _ ("Enter Region Name"),
                      _ ("Enter name for the new Region."), QLineEdit::Normal,
                      dh_info_get_description (DH_TYPE_NBT_INTERFACE_CPP,
                                               uuid));
                  if (str.isEmpty ())
                    QMessageBox::critical (parent, _ ("Error!"),
  ...
  /* Lock NBT fail */
    ...
    QMessageBox::critical (parent, _ ("Error!"), _ ("This NBT is locked!"));
}
...
dh::loadNbtFileAsync (QWidget *parent, QString filedir, bool askForDes)
{
  auto func = [] (GTask *task, gpointer source_object, gpointer task_data,
    ...
    if (!data->des.isEmpty ())
      {
        auto instance
            = new DhNbtInstance (data->filedir.toUtf8 (), data->setFunc,
                                 data->main_klass, task_cancellable, 0, 100);
        if (instance->get_original_nbt ())
          {
            dh_info_new (DH_TYPE_NBT_INTERFACE_CPP, instance,
                         g_date_time_new_now_local (), data->des.toUtf8 (),
                         nullptr, nullptr);
        ...
        else
          {
            ...
            if (g_cancellable_is_cancelled (task_cancellable))
              ewp->error
                  = g_error_new (g_quark_from_static_string ("NBT_LOAD_FAIL"),
    ...
    if (error)
      {
        ...
        QMessageBox::critical (ewp->parent, _ ("Error!"), ewp->error->message);
      }
  ...
  if (askForDes)
    {
      QString qdes
          = QInputDialog::getText (parent, _ ("Enter Description"),
                                   _ ("Enter description for the NBT file."),
                                   QLineEdit::Normal, defaultDes);
      if (qdes.isEmpty ())
