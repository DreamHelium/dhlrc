...
static int download_object(const char* hash, const char* gamedir)
{
    char hash_before[3] = {hash[0], hash[1], 0};
    char* url = g_strconcat(RESOURCES_URL, hash_before, "/", hash , NULL);
    char* dir = g_build_filename(gamedir, "assets", "objects", hash_before, NULL);
    int ret = dh_file_download_full_arg (url, dir, dh_file_progress_callback, (void*)("Object"), TRUE);
    g_free(url);
    g_free(dir);
    return ret;
}

static void cache_translation(const char* filename, std::map<std::string, std::string>& translations)
{
    cJSON* json = dhlrc_file_to_json (filename);
    if (json)
        {
            cJSON* real_json = json->child;
            for (; !g_str_has_prefix(real_json->string, "block"); real_json = real_json->next);
            for (; g_str_has_prefix(real_json->string, "block"); real_json = real_json->next)
                {
                    ...
                    if (strstr(block_name_p, "."))
                        {
                            *strstr(block_name_p, ".") = ':';
                            ...
                            std::string value = cJSON_GetStringValue(real_json);
                            translations.insert(std::pair(name, value));
                            g_free(block);
                        }
                }
            for (; !g_str_has_prefix(real_json->string, "item"); real_json = real_json->next);
            for (; g_str_has_prefix(real_json->string, "item"); real_json = real_json->next)
                {
            ...
            for (auto group : translation_groups)
                {
                    if (group.large_version == large_version)
                        {
                            for (const auto& group_filename : group.filenames)
                                {
...
static char* get_index_dir(const char* gamedir)
{
    ...
    if (auto version_url = get_version_url (version))
        {
            if (dh_file_download_full_arg (
                    version_url, cache_dir, dh_file_progress_callback,
            ...
            if (!g_str_equal (output, "legacy") || !g_str_equal(output, "1.12"))
            ...
            if (!g_file_test(filedir, G_FILE_TEST_IS_REGULAR))
                {
                    if (dh_file_download_full_arg (json_url, dir, dh_file_progress_callback, (void*)("Large Version JSON"), TRUE))
                        {
                            std::cerr << "download large version json failed!" << '\n';
                            cJSON_Delete(json);
                            g_free(filedir);
                            g_free(dir);
                            goto fail_return;
...
char* get_object_hash(const char* gamedir, const char* large_version, const char* object)
{
    ...
    if (gamedir && large_version && object && g_file_test(gamedir, G_FILE_TEST_IS_DIR))
        {
            std::string filename = large_version;
            filename += ".json";
            char* index_dir = get_index_dir(gamedir);
            index_file = g_build_filename(index_dir, filename.c_str(), NULL);
            g_free(index_dir);
        }
...
char* get_object_dir(const char* gamedir, const char* hash)
{
    if (gamedir && hash)
        {
            char hash_before[3] = {hash[0], hash[1], 0};
            char* dir = g_build_filename(gamedir, "assets", "objects",hash_before, hash,NULL);
            return dir;
        }
    return nullptr;
}
