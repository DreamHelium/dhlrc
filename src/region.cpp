  ...
  for (int i = 0; i < len; i++)
    {
      ...
      if (func && main_klass)
        {
          int passed_ms = 1000.0f * (clock () - start) / CLOCKS_PER_SEC;
          if (passed_ms % 500 == 0 || (i + 1) == len)
            {
              char *str = g_strdup_printf ("[%d/%d] %s", i + 1, len,
                                           _ ("Parsing blocks."));
              float progress = (i + 1) / float (len);
              func (main_klass, progress * 100, str);
              g_free (str);
            }
        }
      ...
      if (func && main_klass)
        {
          ...
          if (passed_ms % 500 == 0 || (i + 1) == region_size)
            {
              char *str = g_strdup_printf ("[%d/%d] %s", i + 1, region_size,
                                           _ ("Parsing blocks."));
              func (main_klass, (i + 1) * 100 / region_size, str);
              g_free (str);
            }
        }
...
{
  return region_new_from_nbt_instance_ptr_full (instance_ptr, nullptr, nullptr,
                                                nullptr);
}
...
item_list_new_from_multi_region (const char **region_uuid_arr)
{
  ...
  for (; *region_uuid_arr; region_uuid_arr++)
    {
      Region *region
          = (Region *)dh_info_get_data (DH_TYPE_REGION, *region_uuid_arr);
  ...
  if (g_str_equal (a->id_name, b->id_name))
    {
      ...
      if (a_name->num != b_name->num) /* ?? */
        return FALSE;
      DhStrArray *a_palette = dh_str_array_cat_str_array (a_name, a_data, "=");
      DhStrArray *b_palette = dh_str_array_cat_str_array (b_name, b_data, "=");
      gboolean ret = FALSE;
...
region_get_block_palette (Region *region, int index)
{
  ...
  uint64_t move_num;
  uint64_t end_num;
  gboolean success = g_uint64_checked_mul (&start_bit, index, bits);
  if (success)
    {
      start_state = start_bit / 64;
      and_num = (1 << bits) - 1;
      move_num = start_bit & 63;
      end_num = move_num + bits;
    }
  else
    {
      g_message ("Out of boundary, try another approach.");
      index -= 64;
      start_bit = index * bits;
      start_state = start_bit / 64 + bits;
      and_num = (1 << bits) - 1;
      move_num = start_bit & 63;
      end_num = move_num + bits;
    }
                                       ...
                                       GCancellable *cancellable)
{
  ...
  if (file_is_new_schem (instance_ptr))
    return region_new_from_new_schem_internal (instance_ptr, func, main_klass,
                                               cancellable);
  else
    return region_new_from_nbt_instance_internal (instance, func, main_klass,
                                                  cancellable);
}

Region *
region_new_from_nbt_node (NbtNode *root, DhProgressFullSet func,
