...
{
    module->module_name = g_strdup ("recipe");
    module->module_type = g_strdup ("custom");
    module->module_description = g_strdup ("Recipe Handler");
    module->module_functions = g_ptr_array_new ();
    g_ptr_array_add (module->module_functions, (gpointer)recipe_handler_init);
    g_ptr_array_add (module->module_functions, (gpointer)recipe_handler_free);
    g_ptr_array_add (module->module_functions, (gpointer)recipes_is_supported);
    g_ptr_array_add (module->module_functions,
                     (gpointer)recipes_is_supported_type);
    g_ptr_array_add (module->module_functions, (gpointer)recipes_get_recipes);
    g_ptr_array_add (module->module_functions,
                     (gpointer)pattern_translator_writer);
    g_ptr_array_add (module->module_functions,
                     (gpointer)pattern_translator_free);
    g_ptr_array_add (module->module_functions, (gpointer)recipes_free);
}

void
handler_free (gpointer mem)
{
    Handler *h = mem;
...
void
recipe_handler_init (const char *recipelib_path)
{
            ...
            for (int i = 0; i < g_list_length (filenames); i++)
                {
                    ...
                    GError *err = NULL;
                    char *dir
                        = g_strconcat (recipelib_path, G_DIR_SEPARATOR_S,
                                       g_list_nth_data (filenames, i), NULL);
                    GModule *module
                        = g_module_open_full (dir, G_MODULE_BIND_MASK, &err);
                    g_free (dir);
                    ...
                    if (success)
                        success = g_module_symbol (module, "get_domain",
                                                   (gpointer *)&get_domain);
                    if (success)
                        success = g_module_symbol (module, "get_type",
                                                   (gpointer *)&get_type);
                    if (success)
                        success = g_module_symbol (module, "get_recipes",
                                                   (gpointer *)&get_recipes);
...
gboolean
recipes_is_supported (const char *filename)
{
...
gboolean
recipes_is_supported_type (const char *type)
{
    ...
    for (int i = 0; i < all_handlers->len; i++)
        {
            ...
            if (handler->recipe_domain == NULL
                || g_str_equal (type_sep[0], handler->recipe_domain))
                {
...
DhRecipes *
recipes_get_recipes (const char *filename)
{
    cJSON *json = dhlrc_file_to_json (filename);
    g_return_val_if_fail (json, FALSE);
    char *type = cJSON_GetStringValue (cJSON_GetObjectItem (json, "type"));
...
static void
pt_writer_item_tag (PatternTransformer *pt, cJSON *json)
{
    ...
    if (item)
    ...
    else
        {
            ...
            if (tag)
                dh_str_array_add_str (&(pt->item_string),
                                      cJSON_GetStringValue (tag));
        }
}

void
pattern_translator_writer (PatternTransformer *pt, cJSON *json)
{
...
void
pattern_translator_free (PatternTransformer *pt)
{
    dh_str_array_free (pt->item_string);
}

void
recipes_free (DhRecipes *r)
{
