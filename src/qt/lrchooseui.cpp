  ...
  if (!instance)
    {
      ...
      instance = (DhNbtInstance *)dh_info_get_data (DH_TYPE_NBT_INTERFACE_CPP,
                                                    uuid);
      description = dh_info_get_description (DH_TYPE_NBT_INTERFACE_CPP, uuid);
    }
  ...
  for (int i = 0; i < buttons.length (); i++)
    {
      if (buttons[i]->isChecked ())
        {
          auto des = dh_string_new_with_string (lineEdit->text ().toUtf8 ());
          ...
          Region *region = region_new_from_litematic (
              instance->get_original_nbt (), i, nullptr, nullptr, nullptr);
          dh_info_new (DH_TYPE_REGION, region, g_date_time_new_now_local (),
