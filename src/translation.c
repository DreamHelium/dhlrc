...
void
translation_init (const char *prog_name)
{
    if (!translation_inited)
        {
            ...
            set_locale_to_utf8 ();
            char *locale_dir
                = g_strconcat (dir, G_DIR_SEPARATOR_S, "locale", NULL);
#ifdef LOCALEDIR
...
char *
replace_at_with_slash (char *str)
{
...
char *
get_translation_filedir (const char *prog_name)
{
    char *dir = dh_file_get_current_program_dir (prog_name);
