...
{
    module->module_name = g_strdup ("nbt-reader-qt");
    module->module_type = g_strdup ("qt-shortcut");
    module->module_description = g_strdup ("Qt Nbt Reader");
...
static QString
ret_type_instance (DhNbtType type)
{
    switch (type)
        {
...
static QString
ret_var (DhNbtInstance instance)
{
    ...
    : QWidget (parent), ui (new Ui::NbtReaderUI)
{
    ...
    if (ret)
        {
            auto uuids = dh_info_get_uuid (DH_TYPE_NBT_INTERFACE_CPP);
            instance = *static_cast<DhNbtInstance *> (
                dh_info_get_data (DH_TYPE_NBT_INTERFACE_CPP, (*uuids)[0]));
            ui->setupUi (this);
...
void
NbtReaderUI::addModelTree (DhNbtInstance instance, QStandardItem *iroot)
{
    ...
    if (!selection.isEmpty ())
        {
            ...
            ui->typeLabel->setText (
                ret_type_instance (instance_dup.get_type ()));
...
void
NbtReaderUI::textChanged_cb (QString str)
{
