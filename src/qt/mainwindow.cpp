...
void MainWindow::ilReaderBtn_clicked()
{
    ...
    {
        if(common_info_reader_trylock(DH_TYPE_Item_List, common_info_list_get_uuid(DH_TYPE_Item_List)))
        {
...
void MainWindow::generateBtn_clicked()
{
    ...
    if(ret == QDialog::Accepted)
    {
        ...
        if(!str.isEmpty())
        {
            ItemList* newIl = item_list_new_from_multi_region((const char**)common_info_list_get_multi_uuid(DH_TYPE_Region));
...
void MainWindow::brBtn_clicked()
{
    ...
    if(ret == QDialog::Accepted)
    {
        if(common_info_reader_trylock(DH_TYPE_Region, common_info_list_get_uuid(DH_TYPE_Region)))
        {
