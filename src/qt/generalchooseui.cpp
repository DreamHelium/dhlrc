...
GeneralChooseUI::GeneralChooseUI(DhInfoTypes type, bool needMulti, QWidget *parent) :
    QDialog(parent)
{
...
void GeneralChooseUI::initUI()
{
    ...
    if(len)
    {
        for(int i = 0 ; i < len ; i++)
        {
            ...
            {
                gchar* time_literal = g_date_time_format(common_info_get_time(type, uuid), "%T");
                QString str = QString("%1 (%2)").arg(common_info_get_description(type, uuid)).arg(time_literal);
                QAbstractButton* btn;
                if(needMulti)
        ...
        QObject::connect(okBtn, &QPushButton::clicked, this, &GeneralChooseUI::okBtn_clicked);
        QObject::connect(closeBtn, &QPushButton::clicked, this, &GeneralChooseUI::close);
    ...
    else
    {
        ...
        QObject::connect(okBtn, &QPushButton::clicked, this, &GeneralChooseUI::close);
        setLayout(layout);
    }
}

void GeneralChooseUI::okBtn_clicked()
{
    if(group->checkedId() != -1)
        {
            auto list = const_cast<GList *> (common_info_list_get_uuid_list (type));
            if(!needMulti)
                common_info_list_set_uuid(type, (gchar*)g_list_nth_data(list, group->checkedId()));
            else
                {
                    ...
                    common_info_list_set_multi_uuid(type, (const char**)plain_array);
