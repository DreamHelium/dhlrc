      ...
      if (dirlist)
        {
          for (; dirlist; dirlist = dirlist->next)
            {
              auto realDir = static_cast<const char *> (dirlist->data);
              auto testDir
                  = g_build_path (G_DIR_SEPARATOR_S,
                                  str.toUtf8 ().constData (), realDir, NULL);
              if (g_file_test (testDir, G_FILE_TEST_IS_DIR))
                {
                  auto icontheme = g_build_path (G_DIR_SEPARATOR_S, testDir,
                                                 "index.theme", NULL);
                  if (g_file_test (icontheme, G_FILE_TEST_IS_REGULAR))
                    iconlist << realDir;
                  g_free (icontheme);
                }
              g_free (testDir);
...
MainWindow::initSignalSlots ()
{
  ...
  QObject::connect (ui->scriptBtn, &QPushButton::clicked, this, [&] {
    ...
    if (filename.isEmpty ())
      {
        QMessageBox::warning (this, _ ("Error!"), _ ("No file selected."));
        return;
      }
  ...
  connect (ui->debugBtn, &QPushButton::clicked, [&] {
    ...
    auto region_1 = (Region *)dh_info_get_data (DH_TYPE_REGION, (*allList)[0]);
    auto region_2 = (Region *)dh_info_get_data (DH_TYPE_REGION, (*allList)[1]);
    bool dif = false;
    int i = 0;
