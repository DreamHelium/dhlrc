...
void RecipesUI::recipesInit(QString path)
{
...
void RecipesUI::initUI()
{
    ...
    QObject::connect(getRecipe, &QPushButton::clicked, this, &RecipesUI::getRecipe_clicked);
...
void RecipesUI::okbtn_clicked()
{
    ...
    {
        common_info_new(DH_TYPE_Item_List, new_il, g_date_time_new_now_local(), des2Edit->text().toStdString().c_str());
    }
    else
    {
...
void RecipesUI::getRecipe_clicked()
{
    gchar* recipeDir = dh_get_recipe_dir();
...
static void finish_extract(GPid pid, gint status, gpointer user_data)
{
    ...
    else
    {
        ...
        QString recipeDirQS = QFileDialog::getExistingDirectory(rui, _("Please select a destination."), recipeDir);
        g_free(recipeDir);
        dh_file_copy_dir(sourceDir.toUtf8(), recipeDirQS.toUtf8(), G_FILE_COPY_NONE);
        rui->recipesInit(recipeDirQS);
        rui->drawLayout();
    }
}

void RecipesUI::extract_local(QString srcpath, QString destpath)
{
    ...
    if(unzipDir)
    {
        ...
        g_free(unzipDir);
        if(!g_spawn_async(destdir, arr->val, NULL, G_SPAWN_DO_NOT_REAP_CHILD, NULL, NULL, &pid, &err))
        {
    ...
    else QMessageBox::critical(this, _("No Unzip Found!"), _("No unzip program found!"));
}