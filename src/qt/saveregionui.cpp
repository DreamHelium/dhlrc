                ...
                switch (st.type)
                        ...
                        if (conv_module)
                            {
                                ...
                                QString realFile
                                    = outputDir + G_DIR_SEPARATOR_S
                                      + st.description + ".litematic";
                                temp->save_to_file_full (realFile.toUtf8 (),
                                                         full_set, this,
                                                         cancellable);
                                delete temp;
                            }
                    ...
                    case DHLRC_TYPE_NBT_NO_AIR:
                        if (conv_module)
                            {
                                auto func
                                    = conv_module->module_functions->pdata[6];
                                typedef void *(*RealFunc) (
                                    Region *, gboolean, gboolean,
                                    DhProgressFullSet, void *, GCancellable *);
                                auto real_func
                                    = reinterpret_cast<RealFunc> (func);
                                auto ret
                                    = real_func (st.region, false, ignore_air,
                                                 full_set, this, cancellable);
                                if (ret)
                                    {
                                        auto temp
                                        ...
                                        QString realFile
                                            = outputDir + G_DIR_SEPARATOR_S
                                              + st.description + ".nbt";
                                        temp->save_to_file_full (
                                            realFile.toUtf8 (), full_set, this,
                                            cancellable);
                                        delete temp;
                                    }
                            }
                        ...
                        if (conv_module)
                            {
                                ...
                                auto temp = static_cast<DhNbtInstance *> (
                                    func (st.region, false));
                                QString realFile = outputDir
                                                   + G_DIR_SEPARATOR_S
